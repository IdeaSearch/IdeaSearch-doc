...
<TypeTable
  type={{
    ...
    enable_mutation: {
      description: '是否启用表达式变异操作，增加搜索多样性',
      type: 'boolean',
      default: false,
    },
    enable_crossover: {
      description: '是否启用表达式交叉操作，结合不同表达式特征',
      type: 'boolean',
      default: false,
    },
    optimization_method: {
      description: '参数优化算法选择，L-BFGS-B适合平滑函数，differential-evolution适合复杂函数',
      type: 'string',
      default: 'L-BFGS-B',
    },
    optimization_trial_num: {
      description: '每个表达式的参数优化试验次数，减小可显著提高拟合速度（推荐：5）',
      type: 'number',
      default: 5,
    },
    metric_mapping: {
